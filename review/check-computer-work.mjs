...
const subset={...catalog,professions:catalog.professions.filter(p=>['product_manager','architect','teacher','photographer','courier','barista'].includes(p.id))};
...
let scenario;
for(let i=0;i<100;i++){scenario=createCatalogScenario(subset,[],roster);if(scenario.characters[0].professionRules.id==='product_manager')break;}
assert.equal(scenario.characters[0].professionRules.id,'product_manager');
...
const report={model:await modelStatus(),name:person.name,profession:person.profession,question:'ты работаешь за компьютером?'};
...
await writeFile(new URL('./computer-work-check.json',import.meta.url),JSON.stringify(report,null,2)+'\n');
...
assert.match(report.answer,/компьютер|ноутбук/iu);
